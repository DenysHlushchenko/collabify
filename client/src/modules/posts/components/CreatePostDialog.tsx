...
import { usePostChats } from "@/modules/posts/hooks/usePostChats";
...
const CreatePostDialog = ({ submitPost }: PostDialogProps) => {
  ...
  const { data: chats, isPending, isError } = usePostChats(user?.id);

  const control = form.control as unknown as Control<PostFormInput>;
